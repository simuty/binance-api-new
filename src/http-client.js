...
const responseHandler = res => {
  ...
  const marketName = res.url.includes(FUTURES) || res.url.includes('future') ? 'futures' : 'spot'
...
export default opts => {
  ...
  return {
    ...
    getUserAsset: payload => privCall('/sapi/v3/asset/getUserAsset', payload, 'POST'),
    getIndexInfo: payload => privCall('/fapi/v1/indexInfo', payload),
